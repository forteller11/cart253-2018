...
Paddle.prototype.handleInput = function() { //NEW spelt protoytpe correctly
  ...
  else if (keyIsDown(this.downKey)) { //NEW correctly spelt function keyIsDown, added this to downKey
    this.vy = this.speed; //NEW corrected added to speed (rather than substract)
  }
  else { //NEW made it so paddles now stop moving if keys aren't being pressed
    this.vy = 0;
  }
}
...
Paddle.prototype.update = function() {
  this.y += this.vy;
  this.y = constrain(this.y,0,height-this.h); //NEW spelt height correctly, spelt constrain correctly
}
...
Paddle.prototype.display = function() { //NEW removed extra brackets, NEW spelled display correctly
  rect(this.x,this.y,this.w,this.h); //spelled rect correctly
}
